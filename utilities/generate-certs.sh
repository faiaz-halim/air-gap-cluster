...
echo
 sudo openssl req -x509 -new -nodes -sha512 -days 3650 -subj "/C=CA/ST=CA/L=TORONTO/O=CORP/OU=DEVOPS/CN=${fqdn}"  -key ca.key -out ca.crt
echo
...
echo
 sudo openssl req -sha512 -new \
	 -subj "/C=CA/ST=CA/L=TORONTO/O=CORP/OU=DEVOPS/CN=${fqdn}" \
	 -key ${fqdn}.key \
	 -out ${fqdn}.csr
echo
